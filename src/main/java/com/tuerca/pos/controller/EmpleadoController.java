...
public class EmpleadoController {
    ...
    private void initTablaAcciones() {
        ...
        // Aplicamos el Render y el Editor a la columna 7 (Acciones) — se recorrió
        // de 6 a 7 al agregar la columna "Usuario" (Paso 14)
        vistaGestion.getTablaEmpleados().getColumnModel().getColumn(7).setCellRenderer(new AccionesRender());
        vistaGestion.getTablaEmpleados().getColumnModel().getColumn(7).setCellEditor(new AccionesEditar(evento));
    ...
    private void initListeners(){
        ...
        // Búsqueda con retraso (no consulta la BD en cada tecla), mismo patrón que
        // el resto de pantallas ya migradas
        BusquedaConDebounce.aplicar(vistaGestion.getTxtBuscar(), 300, this::filtrarTabla);

        // AGREGAMOS EL LISTENER DEL MOUSE (columna 7 = Acciones, se recorrió al
        // agregar la columna "Usuario")
    ...
    public void cargarTabla() {
        ...
        Object[] objeto = new Object[8]; // 8 columnas: se agregó "Usuario" (Paso 14)
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            objeto[5] = lista.get(i).getUsername();
            objeto[6] = lista.get(i).getRoleName();
            objeto[7] = "EDITAR / ELIMINAR";
    ...
    private void prepararEdicion(int id) {
        ...
        if (emp != null) {
            vistaEdicion.setNombreField(emp.getNombre());
            vistaEdicion.setPaternoField(emp.getPaterno());
            vistaEdicion.setMaternoField(emp.getMaterno());
            vistaEdicion.setNumeroField(emp.getTelefono());
            vistaEdicion.setUsuarioAsignado(emp.getUsername());
    ...
    private void actualizarEmpleado() {
        ...
        String paterno = vistaEdicion.getPaternoField();
        String materno = vistaEdicion.getMaternoField();
        String numero = vistaEdicion.getNumeroField();
